...
{
  public class CShortPutSpread : CCombination, ICombination
  {
    /// <summary>
    ...
    public override IEnumerable<IScore> GetScores(IEnumerable<IGroup> groups)
    {
      var scores = groups
        ...
        .SelectMany(group =>
        {
          ...
          {
            var groupShort = group.ElementAt(i);

            for (var ii = i + 1; ii < count; ii++)
            {
              var groupLong = group.ElementAt(ii);
              var conditions = new[]
              {
                Equals(groupShort.Option.Bid.CompareTo(groupLong.Option.Ask), 1)
              };
    ...
    public IScore GetScore(IEnumerable<IGroup> groups)
    {
      ...
      return new CScore
      {
        ...
        Credit = CType.ToMoneyStr(optionShort.Bid - optionLong.Ask),
        Symbol = quote.Symbol,
        Distance = CType.ToMoneyStr(quote.Bid - optionShort.Strike),
