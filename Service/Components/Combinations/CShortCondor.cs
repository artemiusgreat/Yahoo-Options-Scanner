...
{
  public class CShortCondor : CCombination, ICombination
  {
    /// <summary>
    ...
    public override IEnumerable<IScore> GetScores(IEnumerable<IGroup> groups)
    {
      var scores = groups
        ...
        .SelectMany(group =>
        {
          ...
          {
            var groupLongUp = group.ElementAt(i);

            for (var ii = i + 1; ii < count; ii++)
            {
              var groupShortUp = group.ElementAt(ii);

              for (var iii = ii + 1; iii < count; iii++)
              {
                var groupShortDown = group.ElementAt(iii);

                for (var iiii = iii + 1; iiii < count; iiii++)
                {
                  var groupLongDown = group.ElementAt(iiii);
                  var conditions = new[]
                  {
                    ...
                    Equals(groupShortUp.Option.Bid.CompareTo(groupLongUp.Option.Ask), 1),
                    Equals(groupShortDown.Option.Bid.CompareTo(groupLongDown.Option.Ask), 1),
                    Equals(groupLongUp.Option.Strike.CompareTo(groupShortUp.Option.Strike), 1),
                    Equals(groupLongDown.Option.Strike.CompareTo(groupShortDown.Option.Strike), -1),
                    Equals(groupShortUp.Option.Strike.CompareTo(groupLongUp.Quote.Ask), 1),
                    Equals(groupShortDown.Option.Strike.CompareTo(groupLongUp.Quote.Bid), -1)
                  };
    ...
    public IScore GetScore(IEnumerable<IGroup> groups)
    {
      ...
      return new CScore
      {
        ...
        Credit = CType.ToMoneyStr(optionLongUp.Ask - optionShortUp.Bid),
        Symbol = quote.Symbol,
        Distance = CType.ToMoneyStr(optionShortUp.Strike - quote.Ask),
